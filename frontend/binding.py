...
class Controlling(QtWidgets.QMainWindow):
    ...
    def fahrt(self, modus):
        ...
        window.Art_der_Fahrt.setEnabled(False)
        window.Datum_feld.setText(self.backend.lbMonitor.date)
        window.Datum_feld.setEnabled(False)
        window.Anfangs_KM_feld.setText(self.backend.lbMonitor.skm)
        window.Anfangs_KM_feld.setEnabled(False)
        if modus == 1 :
    ...
    def fahrtende(self):
        ...
        window.End_KMStand_feld.setEnabled(False)
        window.plus.clicked.connect(self.backend.adjustKmPositive)
        window.plus.clicked.connect(lambda: window.End_KMStand_feld.setText(self.backend.lbMonitor.ekm))
        window.minus.clicked.connect(self.backend.adjustKmNegative)
        window.minus.clicked.connect(lambda: window.End_KMStand_feld.setText(self.backend.lbMonitor.ekm))
